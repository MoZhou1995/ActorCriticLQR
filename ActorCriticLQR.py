...
flags.DEFINE_string('config_path', 'example1.json',
...
def main(argv):
    ...
    #run the algorithm num_run times
    max_norm = np.zeros(5)
    for size in range(6):
        print('size ', size)
        factor = 2**size
        ...
        for index in range(num_run):
            ...
            for t in range(T+1):
                ...
                rho_A_BK = np.max(abs(np.linalg.eigvals(A_BK)))
                ...
                max_norm = np.maximum(max_norm, [rho_A_BK, norm_A_BK, norm_E, norm_K, norm_theta])
                ...
                # when T is large, the first few steps are inefficient because it is too small
                if 2 * t > T: 
                    # actor update
                    K -= beta * natural_grad
                    # critic update
                    theta -= alpha * critic_grad
                else: #boost speed at the beginning
                    # actor update
                    K -= 3 * beta * natural_grad
                    # critic update
                    theta -= 3 * alpha * critic_grad
            train_history = np.array(train_history)
            print('errors', train_history[-1,1:])
            if index == 0:
                saved_history = train_history
            else:
                saved_history = np.concatenate((saved_history, train_history), axis=1)
        # saved_history has size 
        critic = saved_history[:,1:4*num_run:4]
        actor = saved_history[:,2:4*num_run:4]
        critic_mean = np.mean(np.log10(critic), axis=1, keepdims=True)
        actor_mean = np.mean(np.log10(actor), axis=1, keepdims=True)
        critic_std = np.std(np.log10(critic), axis=1, keepdims=True)
        actor_std = np.std(np.log10(actor), axis=1, keepdims=True)
        error_size = np.concatenate((critic_mean, actor_mean, critic_std, actor_std), axis=1)
        if size == 0:
            saved_history_concat = error_size
        else:
            saved_history_concat = np.concatenate((saved_history_concat, error_size), axis=1)
        
    np.savetxt('{}.csv'.format(FLAGS.exp_name),
                saved_history_concat,
                fmt=['%.5e', '%.5e', '%.5e', '%.5e'] * 6,
                delimiter=",",
                # header='step, err_critic, err_actor, err_K,'*(num_run-1) + 'step, err_critic, err_actor, err_K',
                )
    np.savetxt('{}.txt'.format(FLAGS.exp_name + 'norm'), max_norm)
